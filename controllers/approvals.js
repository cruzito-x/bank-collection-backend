...
exports.getApprovals = (request, response) => {
  const approvals =
    "SELECT approvals.approval_id, transactions.transaction_id AS transaction_id, transaction_types.transaction_type, customers.name AS sender_name, customers.email AS sender_email, CONCAT('**** **** **** ', RIGHT(accounts.account_number, 4)) AS sender_account, CONCAT('**** **** **** ', RIGHT(receiver_accounts.account_number, 4)) AS receiver_account, receivers.name AS receiver_name, receivers.email AS receiver_email, transactions.amount, transactions.concept, realizer.username AS realized_by, users.username AS authorized_by, approvals.date_hour AS authorized_at, transactions.date_hour AS datetime, approvals.is_approved FROM transactions INNER JOIN approvals ON approvals.transaction_id = transactions.id INNER JOIN transaction_types ON transaction_types.id = transactions.transaction_type_id INNER JOIN customers ON customers.id = transactions.customer_id INNER JOIN accounts ON accounts.owner_id = customers.id INNER JOIN customers receivers ON receivers.id = transactions.receiver_id INNER JOIN accounts receiver_accounts ON receiver_accounts.owner_id = receivers.id LEFT JOIN users realizer ON realizer.id = transactions.realized_by LEFT JOIN users ON users.id = approvals.authorizer_id ORDER BY datetime DESC";
