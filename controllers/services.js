...
exports.getServices = (request, response) => {
  const services =
    "SELECT services.id, services.service_name AS service, collectors.collector, services.description, services.price, services.deleted_at FROM services INNER JOIN collectors ON collectors.id = services.collector_id WHERE services.deleted_at IS NULL ORDER BY collectors.collector ASC";
